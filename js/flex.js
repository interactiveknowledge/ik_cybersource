(function(Drupal, drupalSettings, once, Flex) {
  Drupal.behaviors.ikCybersourceWebformTemplates = {
    attach: async function(context, settings) {
      once('ikCybersourceWebformSetup', '.webform-submission-form', context).forEach(async function(form) {
        // Sets up the Microform.
        Drupal.behaviors.ikCybersourceWebformTemplates.formName = context.querySelector('.webform-submission-form').getAttribute('title')
        const json = await Drupal.behaviors.ikCybersourceWebformTemplates.fetchToken()
        ...
        number.on('load', function(err) {
          ...
          Drupal.behaviors.ikCybersourceWebformTemplates.log(`Flex microform on ${Drupal.behaviors.ikCybersourceWebformTemplates.formName} loaded`)
        })
        ...
        if (context.querySelectorAll('.is-invalid').length > 0) {
          Drupal.behaviors.ikCybersourceWebformTemplates.log(`Submit ${Drupal.behaviors.ikCybersourceWebformTemplates.formName} error, reason ${context.querySelectorAll('.is-invalid').length} required fields missing`)
        }
        ...
        button.addEventListener('click', Drupal.behaviors.ikCybersourceWebformTemplates.payButton)

        Drupal.behaviors.ikCybersourceWebformTemplates.microform = microform
        Drupal.behaviors.ikCybersourceWebformTemplates.number = number
        Drupal.behaviors.ikCybersourceWebformTemplates.securityCode = securityCode
        ...
        setTimeout(function() {
          if (document.querySelector('#not-loaded-warning') !== null) {
            Drupal.behaviors.ikCybersourceWebformTemplates.log(`Flex microform on ${Drupal.behaviors.ikCybersourceWebformTemplates.formName} did not load after 15s.`)
          }
        }, 15000)
      })
    },
    fetchToken: async function() {
      let webform_id = drupalSettings.ik_cybersource.webform
    ...
    payButton: function(event) {
      ...
      Drupal.behaviors.ikCybersourceWebformTemplates.log(`Submitting ${Drupal.behaviors.ikCybersourceWebformTemplates.formName}`)

      Drupal.behaviors.ikCybersourceWebformTemplates.microform.createToken(options, function(error, token) {
        if (error) {
          ...
          if (error.reason && error.reason === 'CREATE_TOKEN_VALIDATION_FIELDS') {
            ...
            Drupal.behaviors.ikCybersourceWebformTemplates.log(`Submit ${Drupal.behaviors.ikCybersourceWebformTemplates.formName} error, reason ${error.reason}`)
            Drupal.behaviors.ikCybersourceWebformTemplates.log(`User Agent ${navigator.userAgent}`)
            ...
            details.forEach(function(d) {
              if (d.location === 'number') {
                ...
                Drupal.behaviors.ikCybersourceWebformTemplates.number._container.classList.toggle('is-invalid', true)
                Drupal.behaviors.ikCybersourceWebformTemplates.log(`reason ${error.reason}, Validation error. Check that the credit card number is valid.`)
              }
              ...
              if (d.location === 'securityCode') {
                ...
                Drupal.behaviors.ikCybersourceWebformTemplates.securityCode._container.classList.toggle('is-invalid', true)
                Drupal.behaviors.ikCybersourceWebformTemplates.log(`reason ${error.reason}, Validation error. Check that the credit card CVN is valid.`)
              }
            })
          }
            ...
            error.details.forEach(function(detail) {
              ...
              if (location === 'expirationYear') {
                ...
                Drupal.behaviors.ikCybersourceWebformTemplates.log(`reason ${error.reason}, message ${message}`)
          ...
          Drupal.behaviors.ikCybersourceWebformTemplates.log(`Submit Form Flex Microform SUCCESS.`)
    ...
    log: function(message) {
      if (drupalSettings.ik_cybersource.logging === true) {
