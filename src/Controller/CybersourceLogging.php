...
class CybersourceLogging extends ControllerBase {
  ...
  public function logMessage() {
    ...
    if (strlen($requestContent) > 0 && strlen($requestContent) < 255) {
      ...
      if (isset($_SESSION['ik_cybersource']['id']) !== TRUE) {
        $_SESSION['ik_cybersource']['id'] = bin2hex(random_bytes(16));
      }

      $this->logger
        ->get('ik_cybersource')
        ->debug($this->t('":message" from session id :session', [
          ':message' => $message,
          ':session' => $_SESSION['ik_cybersource']['id'],
        ]));
    }
